...
const AdminNavbar = ({ session }: { session: any }) => {
    ...
    return <div className="p-4 select-none flex justify-between bg-white border-[0.1rem] border-zinc-100 relative z-[10]">
        ...
        <div className='flex items-center gap-2'>
            {name != null ? <div className='flex items-center gap-2'><div className={cn("cursor-pointer hover:bg-zinc-50 rounded-xl items-center relative flex justify-center px-2 transition-colors", menuOpened ? "bg-foreground text-white hover:bg-foreground" : "")} onClick={() => {
            ...
            </div>{email == 'abina5448@gmail.com' ? <Link className='bg-green-800 text-white px-4 py-2 rounded-sm' href={'/app/dashboard'}>My Kuttanadu App</Link> : ''}</div> : <div></div>}
