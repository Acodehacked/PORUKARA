...
const UpcomingEvents = ({ events }: {
    ...
    return (
        <>
                ...
                {events.map((event, index) => {
                    return <div key={index} className="bg-zinc-100 border-[0.01rem] border-zinc-500 rounded-md gap-2 p-3 flex items-center justify-between">
                        <div className='flex items-center w-full'>
                            <img src={'/assets/logo-gold.webp'} width={30} height={30} alt='logo' />
                            <div className='ps-3'>
                        ...
                        <div className="w-full flex justify-end gap-2 mt-2">
                            <div onClick={() => seteditDialog(event.id)} className="bg-foreground text-white flex gap-2 items-center px-3 py-1 rounded-md" ><Edit2Icon className="text-white" size={15} /></div>
            ...
            <AnimatePresence>
                {editDialog != -1 && <motion.div transition={{ duration: 0.1 }} variants={GlobalAnimationVariant} initial="hidden" animate="visible" exit="hidden" className='fixed top-0 z-[40] bottom-0 left-0 right-0 flex items-center justify-center bg-[rgba(0,0,0,0.7)]'>
                    <div className='bg-white rounded-lg p-2'>
                        <div className='flex justify-between'>
                            <h2>Alert</h2>
                            <X onClick={() => seteditDialog(-1)} />
                        </div>
                        <div className='px-3 py-6'>
                            <h3 className='text-red-400 font-bold'>Are you sure about deleting this Upcoming Event? </h3>
                        </div>
                        <div className='flex justify-end gap-3'>
                            <Button variant={'secondary'} className='text-black bg-zinc-200' onClick={() => seteditDialog(-1)}>Cancel</Button>
                            <Button onClick={() => DeleteEvent()} variant={'destructive'}>Delete</Button>
                        </div>
                    </div>
                </motion.div>}
