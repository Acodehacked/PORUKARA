        ...
        {filteredList.map((place, index) => {
          ...
          return <div key={place.id} className='relative flex flex-col bg-white rounded-xl overflow-hidden shadow-xl'>
            <div className='w-full me-1'>
              {ENV == 'local' ? <img src={`https://mykuttanadu.s3.us-west-1.amazonaws.com/${(JSON.parse(place.images as unknown as string))[0]}`} className='w-full h-[130px] object-cover' width={200} height={300} alt={place.name || ''} /> : <img src={`https://mykuttanadu.s3.us-west-1.amazonaws.com/${place.images[0]}`} className='w-full h-[130px] object-cover' width={200} height={300} objectFit='cover' objectPosition='center' alt={place.name || ''} />}
            </div>
